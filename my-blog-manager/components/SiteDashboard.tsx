...
export default function SiteDashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch('/api/system/deployed-at', { cache: 'no-store' });
        const json = await res.json();
        if (!cancelled && json?.success && json.deployedAt) {
          const ms = new Date(json.deployedAt).getTime();
          if (!Number.isNaN(ms)) {
            setStartMs(ms);
            return;
          }
      ...
      if (!cancelled) {
        setStartMs(new Date(siteConfig.buildDate || '2026-03-23T00:00:00').getTime());
      }
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    const updateTime = () => {
      ...
      setUptimeStr(formatUptime(now.getTime() - startMs));
    };
