  ...
  const syncPlaylistFromServer = useCallback(async (opts?: { silent?: boolean }) => {
    ...
    try {
      ...
      if (musicIds.length === 0) {
        ...
        setIsPlaying(false);
        setLoadStatus('empty');
        setLoadMessage('尚未配置网易云歌曲，请在管理后台「音乐播放设置」中添加。');
      ...
      if (mergedPlaylist.length > 0) {
        ...
        setLoadMessage(
          `已配置 ${musicIds.length} 首，但无法获取播放地址。请确认是网易云歌曲数字 ID，并在 .env 中配置 NETEASE_APP_ID / NETEASE_APP_SECRET / NETEASE_PRIVATE_KEY。`
        );
